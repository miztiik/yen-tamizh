// ORACLE (contract tier) - the word-ladder Game's import boundary, and the
// no-network claim its result card makes.
//
...
//
// This Game carries one extra structural claim the other five do not: it SHARES
// a result, and it does so with no network call of any kind (Holy Law #1). A
// share endpoint would have been the easy build; it is refused, so the refusal
// is pinned in source here and again in the browser by
// `frontend/tests/word-ladder.spec.ts`, which asserts no request leaves the page
...
const ALLOWED_IMPORTS = new Set([
  ...
  "../../contracts/word-ladder-puzzle", // the generated payload contract (types only)
...
/** Every way a browser can reach the network. None of them may appear (Holy Law #1). */
const NETWORK_APIS: ReadonlyArray<{ pattern: RegExp; api: string }> = [
  { pattern: /\bfetch\s*\(/, api: "fetch()" },
  { pattern: /XMLHttpRequest/, api: "XMLHttpRequest" },
  { pattern: /sendBeacon/, api: "navigator.sendBeacon" },
  { pattern: /\bWebSocket\b/, api: "WebSocket" },
  { pattern: /\bEventSource\b/, api: "EventSource" },
  { pattern: /new\s+Image\s*\(/, api: "new Image() (a pixel is a request)" },
  { pattern: /\bnavigator\.(sendBeacon|connection)\b/, api: "navigator network APIs" },
  { pattern: /https?:\/\//, api: "an absolute URL" },
];
...
describe("word-ladder Game import boundary (Oracle)", () => {
  ...
  it("reads its puzzle only from the payload and its context", () => {
    const view = sources.find((s) => s.name === "WordLadderGame.svelte");
    expect(view).toBeDefined();
    // The view's props ARE the GameContext slice it is allowed to see.
    expect(view?.text).toContain("payload: WordLadderPayload");
...
describe("the result card reaches no network (Oracle)", () => {
  const sources = gameSources();

  it.each(sources.map((s) => s.name))("%s calls no network API", (name) => {
    const source = sources.find((s) => s.name === name);
    // Strip the comments first: this file's own prose names the APIs it bans,
    // and so does the card's, so matching raw text would fail on the ban itself.
    const code = (source?.text ?? "")
      .replace(/\/\*[\s\S]*?\*\//g, "")
      .replace(/(^|[^:])\/\/.*$/gm, "$1")
      .replace(/<!--[\s\S]*?-->/g, "");
    for (const { pattern, api } of NETWORK_APIS) {
      expect(pattern.test(code), `${name} reaches for ${api}`).toBe(false);
    }
  });

  it("shares by building a string on this device, not by calling one down", () => {
    const card = sources.find((s) => s.name === "ShareCard.svelte");
    expect(card).toBeDefined();
    // The whole of "share": a locally built string handed to the clipboard.
    expect(card?.text).toContain("shareText(");
    expect(card?.text).toContain("clipboard");
  });
});
