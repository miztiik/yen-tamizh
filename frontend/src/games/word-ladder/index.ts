// The word-ladder Game's runtime adapter: it binds the Svelte playing surface
// to the `GameModule` contract the SessionRunner drives
...
import WordLadderGameView from "./WordLadderGame.svelte";
import { initialState, type WordLadderPayload, type WordLadderState } from "./logic";

/** The component surface the adapter drives (the view's instance exports). */
interface WordLadderView {
  getState(): WordLadderState;
  restoreState(raw: unknown): void;
  dispose(): void;
}

class WordLadderGame implements GameModule<WordLadderState> {
  private view: WordLadderView | null = null;
  private last: WordLadderState = initialState();

  mount(stage: HTMLElement, ctx: GameContext): void {
    this.view = mount(WordLadderGameView, {
      target: stage,
      props: {
        payload: ctx.payload as WordLadderPayload,
        logger: ctx.logger,
        config: ctx.config,
        now: ctx.now,
      },
  ...
  restoreState(state: WordLadderState): void {
    this.view?.restoreState(state);
  }
}

/** Registry factory: a fresh Game per session item. */
export const wordLadderGameFactory = (): GameModule => new WordLadderGame();
