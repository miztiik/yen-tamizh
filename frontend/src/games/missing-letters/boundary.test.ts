// ORACLE (contract tier) - the missing-letters Game's import boundary.
//
...
const ALLOWED_IMPORTS = new Set([
  ...
  "../../contracts/missing-letters-puzzle", // the generated payload contract (types only)
...
describe("missing-letters Game import boundary (Oracle)", () => {
  ...
  it("reads its puzzle only from the payload and its context", () => {
    const view = sources.find((s) => s.name === "MissingLettersGame.svelte");
    expect(view).toBeDefined();
    // The view's props ARE the GameContext slice it is allowed to see.
    expect(view?.text).toContain("payload: MissingLettersPayload");
