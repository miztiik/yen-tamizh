// ORACLE (contract tier) - the word-search Game's import boundary.
//
...
//
// This Game has two extra things to prove. Its board is several words at once,
// so nothing in it may open a wordlist to decide whether a trace is a word - the
// only words it knows about are the ones its payload names. And it may not reach
// into another Game for the wordle's `MarkShape`, however tempting the reuse
// looks: a Game reaching into a Game is exactly what one-schema-per-Game exists
// to prevent, and that component's vocabulary (correct / present / absent) has
// no meaning on a board whose cells are either settled or not.
...
const ALLOWED_IMPORTS = new Set([
  ...
  "../../contracts/word-search-puzzle", // the generated payload contract (types only)
...
const FORBIDDEN: ReadonlyArray<{ pattern: RegExp; why: string }> = [
  ...
  // Row 20's own rule: the only words this board knows are the ones its payload
...
describe("word-search Game import boundary (Oracle)", () => {
  ...
  it("reads its puzzle only from the payload and its context", () => {
    const view = sources.find((s) => s.name === "WordSearchGame.svelte");
    expect(view).toBeDefined();
    // The view's props ARE the GameContext slice it is allowed to see.
    expect(view?.text).toContain("payload: WordSearchPayload");
