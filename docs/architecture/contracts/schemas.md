...
- **`counters` reconciles every master row under exactly one heading.** `masterRows - outsideLength - outsideBand - invalidWordFinal - withoutCoAnagram - capped == rowsKept == len(words)`, enforced by the model, so a selection bug cannot quietly drop words. Same integrity Oracle as `master-wordlist`'s ingest ledger. (Fowler.)
- **The co-anagram rule is checked against the whole master, not the set being built.** A word qualifies when its ezhuthu multiset is shared with at least one OTHER master word, even one the Game's own selection rejects - the unscramble tension comes from the language, not from the shortlist. The rule is strict and expensive: Tamil has 247 ezhuthu against English's 26 letters, so only 558 of the 50,000 master words (1.1 percent) have any anagram at all, and the anagram set is 163 words rather than the thousands a naive length-and-band filter would yield. Reporting the honest smaller set is the point; weakening the rule would ship scrambles with exactly one possible arrangement. (Palm + Fowler.)
- **`requireValidWordFinal` is the quality rule the co-anagram rule needed (Row 13).** A scraped corpus is full of two things a Tamil speaker would not accept as an ANSWER: sandhi artifacts, where the euphonic doubling belonging to the NEXT word was tokenized onto this one, and transliterated loanwords that keep a final stop. Both END in a mei outside Tamil's eight word-final consonants, so one orthographic test removes them - and because they pair with each OTHER (an inflected form and its own misspelling are anagrams), the test must apply to the co-anagram partner as well, or the tension is satisfied by noise. The rule cut the anagram set from 234 to 163 words. The table of legal word-final ezhuthu lives in the ezhuthu library (`ezhuthu/word_shape.py`) because it is a fact about Tamil letters; the knob that turns it on is config, because whether a Game wants it is a selection decision. (Player + Fowler.)
...
| Dropping co-anagram partners that are an adjacent transposition of the answer | Measured and rejected: it cut the set from 163 to 76 and destroyed REAL pairs, because with three ezhuthu almost every genuine Tamil anagram is an adjacent transposition. It would have removed `arasu`/`asura`. | Player |
