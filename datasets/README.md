...
| `wordlists/derived/` | One `game-wordlist` per Game, cut from the master by `rebuild_wordlists` (Row 9). Build artifacts - regenerated by one command, never hand edited. Committed. |
| `wordlists/by-length/` | The curated `game_words_{2..6}_letter.json` sets carried over from `yen-tamizh_OLD`. A frequency-and-gloss snapshot of the old dictionary, kept as a reference signal; the derived sets are cut from the master, not from these. |
...
- [`../docs/how-to/add-a-derived-wordlist.md`](../docs/how-to/add-a-derived-wordlist.md) - cutting a Game's wordlist out of the master in two steps.
