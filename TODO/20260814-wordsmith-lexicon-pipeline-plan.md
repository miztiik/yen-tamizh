...
| ESCALATE triggers | `delta == full` failing over the STAGED zone after two remediation attempts (row 6); a (`wordClass`, `length`) cell still exceeding 50 MiB after the row-11 decision 6 hash sub-partition (row 11); an unresolved persona conflict; a 3x cost overrun. Rows 3 and 12 change the core data model and what the player is served - the orchestrator reports them prominently but does not block (user waived the Level-5 pause and plan review, 2026-08-14; Fowler, Carmack, Jony, Palm and Player reviewed in their place). |
...
| Runtime environment | The full pipeline runs ON A DEVELOPER LAPTOP, a handful of times over the project's life - not in CI and not on a schedule (user-ruled 2026-08-14). CI runs type checks, tests and the fixture-pipeline gate only (row 11 decision 7); `daily.yml` gains no lexicon step. |
...
| Surface | Name |
| --- | --- |
| The all-words artifact | `lexicon` (`datasets/lexicon/by-class/<wordClass>[-<length>][-<NN>].ndjson` + `datasets/lexicon/lexicon.meta.json`) |
| The processing package | `wordsmith` (`backend/yen_tamizh_backend/wordsmith/`) |
| The LLM gloss / synonym pass | `llm_enrich` |
...
Every word is committed at FULL FIDELITY. Two derived diagnostics - `wordhood` and `freqRank` - are omitted under the row 11 decision 5 principle; every fact a source asserted is published for every word in every class. Files partition by `wordClass`, then by ezhuthu `length` where a class exceeds the size target: roughly 750-840 MB across 10-15 files, none over 50 MiB.

The full rebuild is operator-only. CI runs type checks, tests, the fixture-pipeline integration gate, and a zero-network sha256-set drift check (row 11 decision 7).
...
| A6 | University of Madras Tamil Lexicon (DSAL, not in the old repo) | ~104,000 | headword + English definition | the standard authority; 1930s literary Tamil |
| A7 | Tamil Wiktionary via Wiktextract (not in the old repo) | varies | headword, POS, senses, synonyms | modern coverage the 1930s lexicon lacks |
...
| `gloss` | A1 `en` (12,954), A2 reversed (56,856 English headwords), C1 `english` (1,290), A6, A7, then `llm_enrich` | ~70k rows before enrichment | **sparse -> densified in row 10** |
| `glossSource` | computed | equals gloss coverage | tracks gloss |
| `pos` | A2's POS prefix (`n.`, `adv.`, `a.`), C1's POS-like categories (Verbs, Nouns, Adjectives), A7, B1/B2 as verb evidence | ~57k from A2 alone | **sparse** |
| `synonyms` | A2 grouped by English headword; A7 | ~56,856 English headwords yielding multi-term groups | **sparse** |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Delete `requireCoAnagram`; retain the multiset index as `anagramFanOut` | - | A | PENDING | - | - | - |
| 2 | Move `RelPath` / `SourceId` to `contracts/common.py` | 1 | - | PENDING | - | - | - |
| 3 | `lexicon` + `lexicon-sources` contracts | 2, 4 | - | PENDING | - | - | - |
| 4 | Source acquisition + committed fixtures | - | A | PENDING | - | - | - |
| 5 | `wordsmith/extract.py` + `config/lexicon-sources.json` | 3, 4 | - | PENDING | - | - | - |
| 6 | `wordsmith/stage.py` - the delta store | 5 | - | PENDING | - | - | - |
| 7 | Word-hood exact signals (attestation, orthotactics, breadth) | 6 | B | PENDING | - | - | - |
| 8 | Word-hood inexact signals (n-gram, neighbour, Zipf) | 6 | B | PENDING | - | - | - |
| 9 | `wordsmith/wordhood.py` - the classifier | 7, 8 | - | PENDING | - | - | - |
| 10 | `wordsmith/llm_enrich.py` - gloss + synonym authoring | 9 | - | PENDING | - | - | - |
| 11 | `wordsmith/publish.py` + `pipeline.py` | 10 | - | PENDING | - | - | - |
| 12 | Cut the derived layer over; real serving gates; two-axis difficulty | 1, 11 | - | PENDING | - | - | - |
| 13 | Retire the corpus layer; purge the retired `master` identifiers | 12 | - | PENDING | - | - | - |
| 14 | Rebuild the hint ladder; show a solved word's meaning | 13 | - | PENDING | - | - | - |
| 15 | Themed selection: `categories` + `pos` | 13 | - | PENDING | - | - | - |
...
- **Decisions:**
  ...
  | 6 | Installed-player consequence, flagged not discovered: `.github/workflows/daily.yml` re-bakes and commits the bank nightly and `pick_words` shuffles over the candidate list, so 163 -> 17,313 alters every subsequent bake. The bank is NOT re-baked in this PR; the nightly job picks it up, and row 12 carries the no-rewrite-published-days rule. | Fowler + Palm |
...
- **Decisions:**
  ...
  | # | Decision | Authority |
  | --- | --- | --- |
  | 1 | `RelPath` and `SourceId` are shared vocabulary, not corpus vocabulary, and `contracts/corpus_sources.py` - which row 13 deletes - currently owns them while `derived_wordlists.py` and `game_wordlist.py` import them. Without this move row 13 leaves a dangling import and `mypy` fails. | Fowler |
...
- **Oracle:** The counters model enforces `sum(count per wordClass) == len(words) == the provenance-declared row count` and requires a bucket for every value of the closed enum. Proven by a test that mutates one class count and asserts the model raises. This is the reconciliation that proves the plan's thesis: nothing is discarded.

- **Decisions:**

  | # | Decision | Authority |
  | --- | --- | --- |
  | 1 | `LexiconEntry` row shape, `extra="forbid"`: `word`, `ezhuthu`, `length`, `wordClass`, `wordhood` (a name-keyed map over a closed `SignalName` enum), `attestedBy`, `frequency`, `spokenRatio`, `freqRank`, `compound`, `gloss`, `glossSource`, `pos`, `synonyms`, `categories`, `categorySource`. Every sparse list column is OPTIONAL with `min_length=1`, never a defaulted empty list: `model_dump(exclude_none=True)` drops `None` but not `[]`, so defaulted empty lists would cost roughly 200 MB of nothing against the row-11 budget. | Fowler + user |
  ...
  | 6 | `glossSource` records which source won the gloss, so a bad gloss is traceable to whoever asserted it. It is build-time provenance and is never rendered to a player. | Fowler + Player |
  ...
  | 12 | `categorySource` (enum `attested` / `authored` / `reviewed`) is published, and is REQUIRED because a category renders as a PAID hint. A wrong gloss on the summary is embarrassing; a wrong category the player paid an attempt for and then reasoned from is the game lying and taking payment for it - the worst failure available in this plan. Without this field row 12's `requireReviewedCategory` gate cannot be enforced. | Palm |
...
- **Rejected alternatives:**
  ...
  | 3 | Trust the old curated dictionary's `en` as gloss truth | Measured unreliable: `ஏடு -> "almagest"`, `கயல் -> "dare"` (a carp), `புனல் -> "funnelled"` (water). A wrong meaning shown to a player reads as a broken game, not a wrong dictionary. Admitted at low precedence only. | Fowler + Player |
...
- **Oracle:** Each reader over its real fixture is byte-deterministic across runs, and the extract is lossless: `rows out == rows in - counted parse rejects`.
...
- **Decisions:**
  ...
  | 4 | Facts are typed `headword`, `gloss`, `pos`, `synonym`, `category`, `graphemeCount`; observations are `(source_id, surface, count)`. | Fowler |
  ...
  | 6 | Every reader is a GENERATOR over a bounded buffer, inherited unchanged from `corpus/ingest.py`: line-at-a-time for delimited sources, `JSONDecoder.raw_decode` over a sliding 64 KB buffer for JSON arrays, line-at-a-time for JSONL. No reader may call `json.load`, `read()` or `readlines()` on a source file. Peak RSS of an EXTRACT run over the largest source (D1, 188 MB / 4,591,654 rows) is asserted under 200 MB via `resource.getrusage`, so a whole-file load FAILS the row rather than passing its determinism Oracle. | Carmack |
...
- **Files touched:**
  - `backend/yen_tamizh_backend/wordsmith/signals_exact.py` (new)
...
## 11. Row #10 - `wordsmith/llm_enrich.py` - gloss + synonym authoring

- **Scope:** Batch-author English glosses, Tamil synonyms, part-of-speech tags and categories, and commit the result as an ordinary lexicon source.

- **Files touched:**
  - `backend/yen_tamizh_backend/wordsmith/llm_enrich.py` (new)
  - `config/lexicon-sources.json`
  - `datasets/lexicon/sources/llm-gloss/entries.jsonl` (new, committed)
...
- **Decisions:**
  ...
  | # | Decision | Authority |
  | --- | --- | --- |
  | 1 | The LLM is a SOURCE, not a stage. It runs offline, its output is committed, and the pipeline ingests it like any other dictionary. A model is not reproducible, so it can never sit inside a stage whose Oracle is byte-identity. | Fowler + user |
  | 2 | It authors EVERY published gloss, conditioned on the retained dictionary evidence where evidence exists and unconditioned where it does not. This resolves the inconsistency between row 4 decision 2 (take the fact, never bulk-copy prose) and publishing a lexicographer's definition sentence verbatim: a headword, POS, synonym set and category are facts, a definition sentence is that source's text. `glossSource` records which evidence a gloss was conditioned on. | Fowler, enforcing user's row 4 decision 2 |
  ...
  | 4 | A word whose gloss cannot be authored confidently gets NO gloss rather than a hedged one, and row 12's `requireHumanGloss` gate keeps it out of the served set. A wrong meaning shown to a player reads as a broken game. | Player |
  | 5 | Authored glosses are committed text with per-line diffs, so a bad batch is visible in review and revertible by one commit. | Fowler |
  | 6 | The model client is a named dependency in `[project.optional-dependencies] enrich`, NEVER in `[project.dependencies]`, with its beneficiary and wheel size recorded per Holy Law #8. CI's `pip install -e ".[dev]"` must not pull it: CI runs this module's tests against the committed fixture only, and the module imports its client lazily inside the authoring entry point so an import-time failure cannot break `pytest`. | Carmack |
  | 7 | It authors `pos` on every row it glosses. Dictionary POS covers ~57k of 3.97M surfaces and its overlap with the SERVED headword set is unmeasured; without authored POS, row 15's POS dimension may select nothing. Marginal cost is zero - a model authoring a gloss already determined the part of speech. Decision 4's rule binds unchanged: no confident tag means NO tag. | Fowler + Palm |
  ...
  | 9 | Human review is bounded by CONSUMPTION, not by volume: review only rows that can reach a player - having a category AND passing the serving gates. That is low thousands at most, and about 111 for `themed-nature` today. An afternoon, not a project. Rows a human has reviewed become `categorySource: reviewed`. | Palm |

- **Rejected alternatives:**

  | # | Option | Why rejected | Authority |
  | --- | --- | --- | --- |
  | 1 | Call the model live during PUBLISH | Destroys reproducibility, makes the build network-dependent, and puts a paid API call in CI. | Fowler |
  | 2 | Publish dictionary gloss sentences verbatim | Contradicts row 4 decision 2 - a definition sentence is the source's prose, not a language fact. | Fowler |
...
- **Decisions:**
  ...
  | # | Decision | Authority |
  | --- | --- | --- |
  | 1 | Resolution rules, all config-declared: `frequency` = SUM over frequency-role sources; `spokenRatio` = D9's share of that sum; `attestedBy` = sorted authority sources carrying a `headword` fact; `pos` = UNION over every source carrying a `pos` fact, translated through `posAliases`, deduped and sorted; `synonyms` = UNION; `categories` = UNION normalized through `categoryAliases` so `Birds` and `birds` collapse. `gloss` comes from row 10. | Fowler, Holy Law #6 |
  | 1b | `pos` UNIONS while `gloss` RESOLVES, and the difference is not arbitrary: a gloss occupies ONE display slot so one source must win, whereas `pos` is a set-valued fact with no slot - a Tamil verbal noun is genuinely both, and precedence would delete whichever a lower-ranked source held. `formEvidence` sources B1/B2 contribute `pos: verb`; row 4 decision 1 restricts who may assert WORD-HOOD, and POS is not word-hood - which POS-labels ~1.46M `inflected` rows at zero cost. A wrong tag stays traceable through the store's per-fact `source_id`. Strictness (intersects vs equals) is a SELECTION knob in row 15, never a resolver behaviour. | Fowler |
  | 1a | A source's category value that names a PART OF SPEECH routes to `pos`, never to `categories`. C1's `Nouns` (622), `Verbs` (180) and `Adjectives` (25) are 64 percent of its rows and are not themes; leaving them in `categories` would make `Nouns` the largest "theme" in the lexicon, repeating the blanket-`nouns` mistake already corrected once. The POS-name set and the raw-tag map `posAliases` both live in `config/lexicon-sources.json` beside `categoryAliases`. A raw POS tag with no `posAliases` entry is a HARD PUBLISH FAILURE naming the tag and its row count - never dropped (a silent boundary drop is the defect this plan exists to remove) and never passed through (that defeats the closed enum). CLAUDE.md section 10: fail fast at the boundary. | Fowler + Palm |
  | 2 | Precedence is an explicit per-source integer, not the registry array order, so reordering the array for readability can never silently change a published value. | Fowler, Holy Law #6 |
  | 3 | PUBLISH STREAMS. The lexicon renders as NDJSON - one `json.dumps(row, ensure_ascii=False, sort_keys=True)` per line written straight to a temp handle from a `sqlite3` cursor, then `os.replace`. Peak memory is one row. `render_document` is retained for the SMALL artifacts (bank index, day files, derived wordlists) and is explicitly NOT used for the lexicon: it materializes the joined string three times over, ~12.6 GB at 3.97M rows. That figure is arithmetic about materializing the same content four times, not a property of any machine - hostile on the developer laptop that actually runs this, merely invisible on a larger one. The decisive reason is not memory: streaming is FEWER LINES than materializing, and row 5 decision 6 already forces generator-over-bounded-buffer at EXTRACT, so a materializing PUBLISH would stream at stage 1 and buffer at stage 4 for no gain. The header (`version`, `changelog`, `provenance`, `counters`) is the sibling `lexicon.meta.json`. | Carmack + Fowler |
  | 4 | EVERY word is published at FULL FIDELITY - no class carries a reduced row. Files partition by `wordClass`, and a class over the size target partitions further by ezhuthu `length` (decision 6). Measured: ~3.97M rows at ~212 B (the ~372 B row less `wordhood`) is ~750-840 MB across roughly 10-15 files. The earlier class-appropriate-fidelity rule is REVERSED: a `word`-only line for `inflected` would discard `frequency`, a SOURCE-ASSERTED fact recoverable only from gitignored raw bytes and a URL fetch. That is a real drop, and the directive is literal. | User directive |
  | 5 | Exactly two columns are omitted - `wordhood` and `freqRank` - under a stated PRINCIPLE, not case by case. A column is omitted only if BOTH: (a) it is a DERIVED DIAGNOSTIC of this pipeline, never a fact a source asserted about the word; and (b) omitting it cannot cost the project the fact - either it is recomputable from the COMMITTED ARTIFACT ALONE (`freqRank` is a sort of the published `frequency`), or the VERDICT it produced is published (`wordClass` IS `wordhood`'s verdict, and it is what every consumer reads). `wordhood` is additionally 160 B of a 372 B row - 43 percent of the artifact for eight floats nothing reads. The test is deliberately "from the committed artifact", NOT "from the sources": `frequency` is recomputable from D1-D9, so a source-based test would re-permit the `word`-only line the user rejected. | Carmack + Fowler |
  | 6 | Files partition by `wordClass`, then by ezhuthu `length` for any class over the size target. The target is 50 MiB, NOT the 100 MiB hard limit: 50 MiB is the warning threshold, and designing to the block leaves no headroom for the next refresh. `length` is a stable natural key - a word's cell changes only when its `wordClass` does, which is a real reviewable event. This row MUST report the measured per-(`wordClass`, `length`) byte table in the PR body before the layout is frozen. If any cell still exceeds 50 MiB, that cell alone splits by `sha256(word) % N`, N fixed at authoring time and recorded in `lexicon.meta.json`; a stable hash of the word does not reshuffle when other rows change, which is the property that condemned an arbitrary shard count over row order. | Carmack |
  | 7 | The full rebuild is OPERATOR-ONLY. CI runs `mypy`, `pytest` and the fixture-pipeline integration gate, and nothing else; `daily.yml` gains no lexicon step. Raw sources are gitignored so CI has nothing to rebuild from, `llm_enrich` would need a paid key CI must not hold, and a nightly rebuild would shift frequency sums and therefore the candidate list every night, making row 12's no-rewrite rule unenforceable in principle. CI's drift check is the zero-network sha256-set comparison from row 5 decision 5. | Carmack |
  | 8 | `pipeline.py` only sequences the four stages and holds no logic of its own, so each stage stays independently runnable. | User |
  | 9 | Row ORDER is defined, because dropping `freqRank` removes the total order both byte-identity Oracles relied on (`derive.py` says so in writing). Lexicon NDJSON rows sort by `word` ASC - the store's primary key, therefore total. Sorting by `word` rather than frequency is decision 5's own churn argument: a frequency sort reshuffles nearly every line on any source change, whereas a `word` sort makes a refresh INSERT lines in place, so the diff is the words that actually changed. `derive.py` sorts `frequency` DESC then `word` ASC as the explicit tie-break replacing the removed rank. | Fowler + Carmack |
  | 10 | ~750-840 MB of committed lexicon is accepted: no file exceeds the block, the repo already commits ~265 MB, and a refresh is a handful-of-times event. The named consequence: `daily.yml` checks this repo out nightly and never reads the lexicon, so the workflow adds `sparse-checkout` excluding `datasets/lexicon/` in the same PR - one line, and without it an unrelated job degrades quietly. Git LFS is REJECTED: it makes the files unreviewable in a diff, which is the whole reason NDJSON beat SQLite, and it meters every CI checkout. | Fowler + Carmack |
  | 11 | WHAT THE PARTITION IS OPTIMISING FOR, stated so it is not mistaken for premature optimization. The browser NEVER reads these files; the only consumers are `derive.py` at build time, a human reading a diff, and git storage. So there is exactly ONE hard constraint - GitHub REJECTS any blob over 100 MiB, which is a wall, not a preference - and two soft goals: a diff a human can review, and a read pattern that matches the consumer. `wordClass` then `length` serves both, because `derive.py` asks for exactly "headwords of length 3-6" and that IS a cell address. The 50 MiB target is headroom against the wall, not a performance number, and it costs nothing because the natural split already lands under it. File COUNT is an output of the constraint, never a goal. | Carmack + user challenge |
  | 12 | THE STABILITY ARGUMENT, worked through rather than asserted. Within a file rows sort by `word` ASC, so on a refresh: a new word INSERTS one line; a changed `frequency` or `gloss` rewrites one line IN PLACE; `length` never changes, because it is a property of the word itself; and only a changed `wordClass` moves a row BETWEEN files, which is two line changes and is precisely the semantic event a reviewer should see. Nothing reshuffles. This is why `freqRank` had to go and why the sort is by `word` and not by frequency - a frequency sort rewrites nearly every line whenever any source changes. | Fowler + Carmack |
...
- **Rejected alternatives:**
  ...
  | 4 | Rebuild the lexicon in CI or in `daily.yml` | No raw sources in CI, a paid key CI must not hold, and nightly frequency drift would change already-published days. | Carmack |
  ...
  | 7 | Partition by a hash of the word only | Stable, but it destroys both soft goals: no read locality for `derive.py`, and "what changed in headwords?" would span every file. It survives only as the decision-6 fallback for a single oversized cell. | Carmack |
...
- **Oracle:** The regenerated `anagram.json` byte-equals a fresh re-derive; `அசுர`, `திமுக` and `ஸ்டாலின்` are all absent; every served row satisfies all four gates; and no already-published bank day is rewritten.

- **The serving gates** (PRESENT is everything; SERVED must satisfy all five):

  | Gate | Shipping default | Why |
  | --- | --- | --- |
  | `wordClass` | `["headword"]` | Production vs recognition - see decision 1 |
  | `minAttestations` | `2` | Single-authority headwords are where 1930s dead vocabulary and model inventions live |
  | `minFrequency` | `1` | A dictionary word appearing zero times in 3.9M words of modern Tamil is a museum piece. Does the most work of the five |
  | `requireHumanGloss` | `true` | A word only a model can gloss is a word nobody should be served |
  | `requireReviewedCategory` | `true` | An unreviewed model-authored category may sit in the lexicon for exploration, but it may not render as a PAID hint or select a themed round. A wrong category the player paid an attempt for and reasoned from is the game lying and taking payment for it |
...
- **Decisions:**
  ...
  | 8 | The no-rewrite rule is ENFORCED IN CODE, not by convention. `scripts/generate_today.py` currently loops `dates_from(start, daysAhead)` and calls `write_artifact` UNCONDITIONALLY on all seven days; it is idempotent today only because the wordlist and seed happen to be unchanged. The moment this row lands a new `anagram.json`, the next `daily.yml` cron tick rewrites today..today+6 from the new candidate list. `generate()` therefore skips any date whose file already exists unless `--rebake` is passed, while the bank index is still rebuilt from disk every run so it cannot drift. A test asserts that re-running after mutating the derived wordlist leaves every pre-existing day file byte-identical. | Carmack |
  | 9 | With decision 8 in place NO service-worker cache-bust is needed, and this is recorded so it is not re-litigated: `vite.config.ts` `globPatterns` excludes `json`, so no bank file is precached; the bank is runtime-cached `StaleWhileRevalidate`; every new day is a new URL and therefore a cache miss that fetches correct bytes on first open. | Carmack |
  | 8 | `requireValidWordFinal` and its `invalidWordFinal` counter bucket are deleted here, superseded by row 7's signal 2. `DerivedSource.generatedAt` is dropped. `masterPath` -> `lexiconPath`, `DerivedCounters.masterRows` -> `lexiconRows`, `load_master` -> `load_lexicon`, `_MASTER` fixtures renamed. | Fowler |
  | 9 | Band-based selection is removed entirely, replaced by the absolute `minFrequency` floor plus the stratified draw. | Palm |
  | 10 | `_SCHEMA_VERSION` is date-stamped with a `changelog` entry naming every removed and added field, in the same commit. | Fowler, CLAUDE.md section 11 |
...
- **Decisions:**
  ...
  | 4 | `datasets/wordlists/by-length/` is retained unchanged as a reference signal - already ruled not a hard filter. | Palm (existing ruling) |
...
- **Files touched:**
  ...
  - `frontend/src/shell/DailySession.svelte`, `frontend/src/session/types.ts`
  - `schemas/**`, `frontend/src/contracts/**` (generated)
...
- **Decisions:**
  ...
  | 4 | ENGLISH IS BANNED ON THE STAGE. The meaning rung resolves Tamil synonym -> Tamil gloss -> the rung is OMITTED from the baked ladder entirely. There is no English fallback on a paid hint: a hint the player cannot read is a hint that stole score. A ladder that is sometimes two rungs is correct. | Jony + Player |
  ...
  | 10 | `GameWord` gains `gloss`, `synonyms` and `categories`. `AnagramPuzzle` gains exactly TWO new optional fields: `meaning` (an already-resolved display string) and `alsoValid` (a list of accepted alternative words). The arrays stay build-time - the generator resolves them into rendered text and the player downloads the text, not the inputs, the same rule as `build_item` dropping the per-item schema stamp. | Carmack + Jony |
  | 11 | `alsoValid` MUST be baked. `anagramFanOut` (row 1) is a count on a build-time wordlist row, the Game cannot detect the third state from a count, and runtime wordlist consumption is forbidden - so an unbaked affordance could never fire. | Jony |
  ...
  | 14 | `SessionResult` gains a per-item `solved` flag. Today it carries only counts and `puzzle.completed` carries no word, so the summary cannot tell a solved word from a lost one - and an unmarked block would have to be labelled neutrally rather than as a scorecard. | Jony |
...
- **Files touched:**
  ...
  - `backend/yen_tamizh_backend/contracts/derived_wordlists.py`
  - `config/derived-wordlists.json`, `config/copy.json`
...
- **Decisions:**
  ...
  | 3 | THE THEMES ARE TINY, and the plan says so rather than discovering it in execution. After routing the POS labels to `pos` (row 11 decision 1a), C1 leaves 463 rows across 37 themes: the largest is Emotions at 35, and Birds is 12, Colours 7, Flowers 4, Amphibians 2. A single-theme Daily at 3/day is **1 to 12 days**, not the 71 estimated before the categories were counted. | Palm, corrected by measurement |
  ...
  | 10 | RECORDED WARNING, because the capability is shipped on user directive against Palm's design finding: no currently planned Game is improved by a POS restriction. Tamil verb HEADWORDS are 1-3 ezhuthu (`வா`, `போ`, `செய்`, `படி`), so a verbs-only anagram day drives the generator into the shortest band - which row 12 decision 5 identifies as the brutal tail. A verbs day is not a neutral variant; it is the hardest possible day, produced by accident. Adjectives number 25 rows. Nouns exclude nothing. POS's load-bearing jobs remain build-time: `wordClass` input, and synonym-set integrity (without it, a noun sense and a verb sense sharing one English headword collapse into one synonym set and row 14's `meaning` rung resolves to a wrong-POS synonym). | Palm |
