...
| Surface | Name |
| --- | --- |
| The all-words artifact | `lexicon` (`datasets/lexicon/by-class/<wordClass>-<length>[-<hex>].ndjson`, where `<length>` is UNCONDITIONAL and `<hex>` is the lowercase 4-digit hex of the word's base first ezhuthu, present only on cells split at first publish. `lexicon.meta.json` carries `partitionKeys`, a `partitions[]` entry per file, and an `ezhuthuIndex` decoding every hex to its Tamil character, romanization and kind - see row 11 decision 15) |
...
**1. Proper nouns and grammar words - PARTLY FIXED in row 16, the rest still open.** Row 16 shipped `config/served-denylist.json`: 55 function words and 14 names, curated from the actual top of the served set and removed from serving only. That takes the worst of it off the board - the three most-played words were "that", "this" and "and". WHAT REMAINS: the deny-list was seeded from the top 400 by frequency, so names below that are still served, and the general problem is unsolved. A part-of-speech rule was measured and REJECTED - `pos` is a union across 21 sources, so `அப்பா` (father) and `அரசு` (government) both carry `interjection` and a POS filter deletes real vocabulary while still missing `குமார்`, which is tagged only `noun`. Tamil Wikipedia titles were also acquired and measured insufficient: the full table is in [`../docs/architecture/lexicon/word-hood.md`](../docs/architecture/lexicon/word-hood.md), ~32% precision - two ordinary words deleted per entity caught. Untried: Wikidata `instance of: human`; a name-suffix signal (`-வேல்`, `-ராஜ்`, `-லிங்கம்`, `-நாத்`); extending the deny-list down the frequency curve, which is cheap and needs no new mechanism.
...
- **Decisions:**
  ...
  | 2a | **SUPERSEDES the last clause of decision 2 (user-ruled 2026-08-16).** A Tamil DEFINITION may now be published VERBATIM as `meaningSource: attested`. The user's reasoning, recorded because it governs every future source: Tamil predates corporate law and digitisation, so no modern party owns its words or their meanings, and asserting such ownership is cultural misappropriation rather than a licence. No commercial republication is occurring. The original clause was written when NO Tamil-definition source existed and `llm_enrich` authoring was the only route to a meaning; row 4b's `ta-wiktionary-content` (A9) supplies 146,527 attested Tamil meanings, 77,558 of them on `headword` rows, so the clause now blocks the best data in the inventory for no benefit. Practical effect: `meaningSource: attested` is a first-class published value ranked ABOVE `authored`, and `llm_enrich` fills only what no source attests. | User |
...
- **Decisions:**
  ...
  | 15 | **NAMING AND THE META INDEX (Carmack ruling, 2026-08-15, on a user challenge, from MEASURED data).** Pattern is `<wordClass>-<length>[-<hex>].ndjson`. THREE amendments to decision 8: (a) **`<length>` is UNCONDITIONAL**, not applied only to a class over target - a conditional length makes a word's ADDRESS depend on the SIZE OF ITS CLASS, a property of the artifact rather than of the word, which embeds a forced future rename (`properNoun.ndjson` -> `properNoun-3.ndjson`) that decisions 11 and 14 forbid; unconditional costs nothing and leaves exactly ONE conditional component. (b) **The pre-emptive split threshold is 33 MiB, not 50** - one third of GitHub's 100 MiB HARD WALL (50 MiB is only the warning line), so any cell 3x lifetime growth would push through the wall splits now. The cost asymmetry decides it: an unnecessary split costs ~34 files in a directory whose median file is 7 KiB, while a missed split costs the forbidden re-split. 33 MiB also sits in the measured distribution's WIDEST GAP (18.5%), so the line is not drawn mid-cluster. Six cells split (`inflected` 7-11, `unclassified` 11); ~639 files becomes ~840. (c) **Filenames stay HEX, never romanized and never Tamil script.** Tamil script is disqualified by CLAUDE.md section 5 AND because git's default `core.quotepath=true` renders non-ASCII paths as octal escapes - it is LESS legible than hex on the operator's own Windows machine. Romanization is disqualified because ISO 15919 is not ASCII (it is diacritic-based) and ITRANS is CASE-SIGNIFICANT (`N`/`n`, `L`/`l`), which COLLIDES on case-insensitive NTFS and APFS - and this repo's `word_shape.py` annotations already contain that exact collision. The rule underneath: **put the IMMUTABLE identifier in the path and the CORRECTABLE label in data.** A Unicode codepoint is fixed by an external standard; a romanization is a judgement call, and correcting one would rename published files. The repo's existing ASCII spelling of a Tamil character IS the hex codepoint (`ezhuthu/` is written in `\u0b95` escapes throughout), so hex speaks the convention already in use rather than minting a second one. Note the hex suffix lands on only six cells, ALL inside classes with no `derive.py` reader - `headword-4.ndjson` never carries one. | Carmack |
  | 16 | **`lexicon.meta.json` IS the index, and it is where legibility is answered.** It carries `partitionKeys` (the ordered address `["wordClass", "length", "baseFirstEzhuthu"]`, declared in the artifact so the address function does not live only in the code that wrote it); `partitions[]`, ONE ENTRY PER FILE with `path`, `wordClass`, `length`, `baseFirstEzhuthu` (**present only on split cells - its absence IS the not-split signal**), `rows`, `bytes`, `sha256`; and **`ezhuthuIndex`**, decoding each hex ONCE to `{ezhuthu: <the Tamil character>, roman, kind}`. The Tamil character and its romanization are therefore SPELLED OUT for a human - as correctable data in a JSON value a reviewer already opens, never as an immutable path component, so a romanization fix is a one-line diff plus a changelog entry rather than a rename. Resolution contract: a cell is split IFF at least one `partitions[]` entry for that (`wordClass`, `length`) carries `baseFirstEzhuthu`; to read a whole cell, select every matching entry, order by `baseFirstEzhuthu` ASC and concatenate - which reproduces the sorted cell, making the split invisible to a consumer. NO probe-and-fallback (that is globbing wearing a different hat, forbidden by row 12 decision 10). NO derived `splitCells` list - recomputable from `partitions[]`, and a second source of truth that can disagree with the first is a defect waiting to happen. EXTEND THE ORACLE: `sum(p.rows for p in partitions) == counters.rows` and the per-class sums match `counters.byClass` - this catches a file written but not registered, or registered but not written, which is the one failure a no-globbing reader is structurally blind to. | Carmack |
...
- **Decisions:**
  ...
  | 10 | `GameWord` gains `definitionTa`, `translationEn`, `synonymsTa` and `categories`. `AnagramPuzzle` gains THREE new optional fields, every one of them an already-RESOLVED display value: `meaning` (a Tamil display string), `translationEn` (the summary's demoted English second line) and `alsoValid` (a list of accepted alternative words). The arrays stay build-time - the generator resolves them into rendered text and the player downloads the text, not the inputs, the same rule as `build_item` dropping the per-item schema stamp. **AMENDED 2026-08-17 (user-ruled): "exactly TWO" became three.** As written, decision 5 wanted an English second line on the summary while nothing on the payload could carry it, so `GameWord.translationEn` would have shipped 18,268 rows of committed data with no reader. `translationEn` rides the same rule as `meaning` - resolved at bake time, never an array, never sold as a hint - so the count changed and the rule did not. | Carmack + Jony |
...
- **DECISION 19 IS WITHDRAWN (user-ruled 2026-08-17). Do not implement it, and do not reinstate it without new evidence.**

  It was authored on 2026-08-14 by a planning session and attributed to Palm + Fowler - both AI personas. The user never made this rule, and it was wrongly presented back to them as a constraint they had to resolve. That is the mistake being recorded here, more than the rule itself.

  It also could not have run: it reads `meaningSource` and `categorySource`, which row 11 deleted from the published row at the user's instruction. `ProvenanceState` remains declared in `contracts/lexicon.py` and re-exported in `__all__` while nothing assigns or reads it - a dead type. A worker implementing decision 19 as written would have shipped a rule that silently no-ops.

  **Then it was measured, and the concern did not survive contact with the data.** Of the 32,310 words a player can be served, **1,210 (3.7%)** would display a meaning the authored pass wrote. Reading them, they are accurate, well-formed Tamil - `இந்த` "the word that points to what is near or is being spoken of", `மற்றும்` "the particle that joins two or more things", `ஆறாத` "that does not subside". A rule guarding a 3.7% slice that reads correctly does not earn the mechanism it would cost.

  Row 14 therefore ships WITHOUT a reviewed/unreviewed distinction: every published meaning renders and may be sold. `ProvenanceState` should be DELETED in the same row, because a closed vocabulary nothing assigns is a comment wearing a type.

  What the measurement DID find is recorded as open defect 1 below and fixed in row 16: the words carrying those authored meanings were disproportionately grammar words and names, which is a far worse defect than who wrote the gloss.
