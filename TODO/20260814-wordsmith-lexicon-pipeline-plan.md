...
| Hard scope - in | Delete `requireCoAnagram` while retaining its index as a difficulty signal; build the `wordsmith` package as four independently runnable stages (EXTRACT / STAGE / ENRICH / PUBLISH) supporting delta and atomic per-source update; classify every word by an eight-signal word-hood judgement into a closed `wordClass` enum; retain gloss, POS, synonyms, categories, frequency; rename the `master` surface to `lexicon`; LLM gloss authoring as a committed source; cut the derived layer over with real serving gates; retire the corpus layer; rebuild the hint ladder and show a solved word's meaning; ship themed selection. |
