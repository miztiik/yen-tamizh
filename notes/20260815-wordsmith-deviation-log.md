...
| D12 | **Attested Tamil definitions may be published VERBATIM.** Supersedes row 4 decision 2's "never republish a source's definition sentence" clause. | Tamil predates corporate law and digitisation; no modern party owns its words or meanings, and claiming so is cultural misappropriation rather than a licence. No commercial republication is occurring. The clause was written when no Tamil-definition source existed; A9 now supplies 146,527. | User, 2026-08-16 |
| D13 | **Google Translate rejected on evidence, not preference.** No bulk artifact exists (billing-keyed REST API only) and its terms forbid redistributing output as a dictionary. Nothing was scraped. IndoWordNet (IIT Bombay CFILT) adopted instead - real Tamil synsets with glosses, keyed to Princeton WordNet sense ids. | The user's requirement was two corroborating sources; IndoWordNet delivers sense-scoped synonyms, which is exactly the field A2's sideways read broke. | Row 4b research + User |
| D14 | **Row 4b is the single largest content win of the plan.** Headwords with a real Tamil meaning **6,269 -> 77,558** (12.4x); in the 3-6 ezhuthu band **6,269 -> 55,133** of 85,314 against row 12's floor of 6,000. Also surfaced 13,500 `unclassified` surfaces that have a Wiktionary page WITH a meaning - direct evidence the classifier got them wrong. | The user's instruction to check EVERY class, not just headwords. | User |
