...
from yen_tamizh_backend.wordsmith.publish import BY_CLASS, META_NAME
...
def rebuild(repo_root: Path, workspace: Path) -> list[Path]:
    ...
    run(scoped, config, workspace, workspace / "out" / "cache" / "lexicon.db", force=True)

    target = repo_root / EXPECTED
    if target.exists():
        shutil.rmtree(target)
    (target / BY_CLASS).mkdir(parents=True)
    written: list[Path] = []
    for path in sorted((workspace / "out" / BY_CLASS).glob("*.ndjson")):
        shutil.copyfile(path, target / BY_CLASS / path.name)
        written.append(target / BY_CLASS / path.name)
    shutil.copyfile(workspace / "out" / META_NAME, target / META_NAME)
    written.append(target / META_NAME)
    return written
