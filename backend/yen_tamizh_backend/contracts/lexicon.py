...
of the WHOLE population the store holds, and ``published`` is what the files
actually carry. That pair is what proves the artifact's thesis - the classes a
publish policy withholds are still counted in the repository, so nothing is
discarded, only unpublished.
"""
...
# immutable per word - the class is the classifier's verdict and the base first
# ezhuthu is the word's own first code point - so a refresh INSERTS into a file
# and only a changed verdict ever moves a row between files.
PARTITION_KEYS: Final[tuple[str, ...]] = ("wordClass", "baseFirstEzhuthu")
...
LEXICON_VERSION = "2026-08-16T10:00"
LEXICON_CHANGELOG = (
    ChangelogEntry(
        version=LEXICON_VERSION,
        change=(
            "Split counters into a classified and a published census; made "
            "baseFirstEzhuthu the second and final partition key and dropped "
            "length from the partition table; added partitionKeys and "
            "ezhuthuIndex; dropped the ezhuthu column from the row; renamed "
            "provenance rowsIn / factsOut to observations / facts."
        ),
        why=(
            ...
            "class, so 'nothing was discarded' is a checkable statement rather "
            "than a claim. The measured layout is one file per (wordClass, base "
            "first ezhuthu) at 241 files and a 2.2 MiB largest file, so the "
            ...
            "there. ezhuthu goes for the reason freqRank went: it is "
            "segment(word), a pure function of a published column, and storing "
            "a derived value is a drift surface as well as 35.4 B a row. "
            "observations / facts are what the store can actually prove about a "
            "source; rowsIn read as raw rows and no stage keeps that."
        ),
    ),
    ChangelogEntry(
        version="2026-08-16",
...
class LexiconEntry(BaseModel):
    ...
    def _length_is_the_word_own_ezhuthu_count(self) -> Self:
        ...
        # the column go without the check going with it.
...
class LexiconPartition(BaseModel):
    ...
    path: RelPath
    wordClass: WordClass
    baseFirstEzhuthu: str = Field(pattern=_HEX4)
    rows: int = Field(ge=0)
    bytes: int = Field(ge=0)
    sha256: str = Field(pattern=_SHA256)
...
class Lexicon(SchemaModel):
    ...
    def _every_partition_key_decodes_through_the_index(self) -> Self:
        ...
        for hex_key, entry in self.ezhuthuIndex.items():
            expected = f"{ord(entry.ezhuthu):04x}"
            if hex_key != expected:
                raise ValueError(
                    f"ezhuthuIndex key {hex_key!r} != {expected!r} for "
                    f"{entry.ezhuthu!r}"
                )
        used = {cell.baseFirstEzhuthu for cell in self.partitions}
